...
a.model.generateCardCountDown=GENERATE_CARD_INTERVAL),a.model.generateCardCountDown--,a.model.totalTime++)});this.schedule(this.schedulePerSec,1)},generateCards:function(){var a=this.model.getPattern(),b=_.sample([0,1]),c=_.sample([0,1]),a=a.get("list");_.each(a,function(a){var e=this.model.drawCard();if(null!=e){var f=new PokerCardSprite({model:e});f.attr({x:b?cc.winSize.width-a.start.x:a.start.x,y:a.start.y});this.addChild(f);var g=a.moveTime;14===e.get("number")&&(g/=2);f.runAction(new cc.sequence(new cc.delayTime(a.time),
new cc.moveTo(g,b?cc.winSize.width-a.end.x:a.end.x,a.end.y),new cc.callFunc(function(){gameModel.destroyCard(e)},this)));var h=this.model.drawCard();if(null!=h){var f=new PokerCardSprite({model:h}),k,m;c?(f.attr({x:a.start.x,y:cc.winSize.height-a.start.y}),k=a.end.x):(f.attr({x:cc.winSize.width-a.start.x,y:cc.winSize.height-a.start.y}),k=cc.winSize.width-a.end.x);m=cc.winSize.height-a.end.y;this.addChild(f);g=a.moveTime;14===h.get("number")&&(g/=2);f.runAction(new cc.sequence(new cc.delayTime(a.time),
new cc.moveTo(g,k,m),new cc.callFunc(function(){gameModel.destroyCard(h)},this)))}}},this)},initAudio:function(){var a=cc.sys.localStorage.getItem("sound");this.sound=null!=a?a:0.5;cc.audioEngine.setEffectsVolume(this.sound)},gameOver:function(){cc.audioEngine.playEffect(res.game_over_mp3,!1);this.betRateLabel1.setString("Game Over");this.betRateLabel2.setString("Game Over");this.betRateLabel1.runAction(new cc.spawn(new cc.moveTo(times.gameOver,cc.winSize.width/2,cc.winSize.height/2),new cc.scaleTo(times.gameOver,
2,2)));this.betRateLabel2.runAction(new cc.spawn(new cc.moveTo(times.gameOver,cc.winSize.width/2,cc.winSize.height/2),new cc.scaleTo(times.gameOver,2,2)));this.handTypeLabel1.setVisible(!1);this.handTypeLabel2.setVisible(!1);this.player1Sprite.moneyLabel.runAction(new cc.spawn(new cc.moveTo(times.gameOver,cc.winSize.width/2,cc.winSize.height/2-250),new cc.scaleTo(times.gameOver,2,2)));this.player2Sprite.moneyLabel.runAction(new cc.spawn(new cc.moveTo(times.gameOver,cc.winSize.width/2,cc.winSize.height/
2+250),new cc.scaleTo(times.gameOver,2,2)));cc.eventManager.addListener(cc.EventListener.create({event:cc.EventListener.TOUCH_ONE_BY_ONE,swallowTouches:!0,onTouchBegan:function(a,b){return!0},onTouchMoved:function(a,b){},onTouchEnded:function(a,b){cc.director.runScene(new IntroScene)}}),this)}}),GameModel=Backbone.Model.extend({initialize:function(){this.countDown=this.maxCountDown=60;this.generateCardCountDown=0;this.set("betRate",1);this.set("status","ready");this.totalTime=0;this.cidToModel={};
this.player1=new PlayerModel({position:PLAYER_POSITION_DOWN,playerType:"player"});this.player2=new PlayerModel({position:PLAYER_POSITION_UP,playerType:"player"});this.patternPool=[new PatternModel,new Pattern2Model,new Pattern3Model,new Pattern4Model,new Pattern5Model,new Pattern6Model]},newDeck:function(){this.deck=newDeck();this.discardDeck=[]},drawCard:function(){var a=this.deck.pop();a||(this.deck=_.shuffle(this.discardDeck),this.discardDeck=[],a=this.deck.pop());a&&(this.cidToModel[a.cid]=a,
a._owned=!1);return a},clearNotOwnedCards:function(){var a=[];_.each(this.cidToModel,function(b){null==b||b._owned||a.push(b)},this);_.each(a,function(a){this.destroyCard(a)},this)},clearCards:function(){var a=[];_.each(this.cidToModel,function(b){null!=b&&a.push(b)},this);_.each(a,function(a){this.destroyCard(a)},this);this.cidToModel={}},destroyCard:function(a){this.discardDeck.push(new PokerCardModel({number:a.get("number"),suit:a.get("suit")}));a.destroy();delete this.cidToModel[a.cid]},getPattern:function(){return _.sample(this.patternPool)}}),
