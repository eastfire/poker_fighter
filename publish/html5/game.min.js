...
k;a[6]=c;a[7]=g;return this};b.determinant=function(){var a=this.mat,b=a[0]*a[4]*a[8]+a[1]*a[5]*a[6]+a[2]*a[3]*a[7];return b-=a[2]*a[4]*a[6]+a[0]*a[5]*a[7]+a[1]*a[3]*a[8]};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var g=b[1],h=b[2],k=b[3],m=b[4],p=b[5],r=b[6],t=b[7],u=b[8],x=c[0],w=c[1],E=c[2],n=c[3],M=c[4],B=c[5],D=c[6],N=c[7],c=c[8];b[0]=a*x+k*w+r*E;b[1]=g*x+m*w+t*E;b[2]=h*x+p*w+u*E;b[3]=h*x+p*w+u*E;b[4]=g*n+m*M+t*B;b[5]=h*n+p*M+u*B;b[6]=a*D+k*N+r*c;b[7]=g*D+m*N+t*c;b[8]=h*D+p*N+u*c;
...
return this};b.get=function(a,b){return this.mat[a+4*b]};b.set=function(a,b,c){this.mat[a+4*b]=c};b.swap=function(a,b,c,d){var k=this.mat,m=k[a+4*b];k[a+4*b]=k[c+4*d];k[c+4*d]=m};a.math.Matrix4._gaussj=function(a,b){var c,d=0,k=0,m,p,r,t,u=[0,0,0,0],x=[0,0,0,0],w=[0,0,0,0];for(c=0;4>c;c++){for(m=t=0;4>m;m++)if(1!==w[m])for(p=0;4>p;p++)0===w[p]&&(r=Math.abs(a.get(m,p)),r>=t&&(t=r,k=m,d=p));++w[d];if(k!==d){for(m=0;4>m;m++)a.swap(k,m,d,m);for(m=0;4>m;m++)b.swap(k,m,d,m)}x[c]=k;u[c]=d;if(0===a.get(d,
d))return!1;p=1/a.get(d,d);a.set(d,d,1);for(m=0;4>m;m++)a.set(d,m,a.get(d,m)*p);for(m=0;4>m;m++)b.set(d,m,b.get(d,m)*p);for(p=0;4>p;p++)if(p!==d){r=a.get(p,d);a.set(p,d,0);for(m=0;4>m;m++)a.set(p,m,a.get(p,m)-a.get(d,m)*r);for(m=0;4>m;m++)b.set(p,m,a.get(p,m)-b.get(d,m)*r)}}for(m=3;0<=m;m--)if(x[m]!==u[m])for(p=0;4>p;p++)a.swap(p,x[m],p,u[m]);return!0};var c=(new a.math.Matrix4).identity();a.kmMat4Inverse=function(b,d){var g=new a.math.Matrix4(d),h=new a.math.Matrix4(c);if(!1===a.math.Matrix4._gaussj(g,
h))return null;b.assignFrom(g);return b};b.inverse=function(){var b=new a.math.Matrix4(this),d=new a.math.Matrix4(c);return!1===a.math.Matrix4._gaussj(b,d)?null:b};b.isIdentity=function(){var a=this.mat;return 1===a[0]&&0===a[1]&&0===a[2]&&0===a[3]&&0===a[4]&&1===a[5]&&0===a[6]&&0===a[7]&&0===a[8]&&0===a[9]&&1===a[10]&&0===a[11]&&0===a[12]&&0===a[13]&&0===a[14]&&1===a[15]};b.transpose=function(){var a=this.mat,b=a[1],c=a[2],d=a[3],k=a[6],m=a[7],p=a[8],r=a[9],t=a[11],u=a[12],x=a[13],w=a[14];a[1]=a[4];
a[2]=p;a[3]=u;a[4]=b;a[6]=r;a[7]=x;a[8]=c;a[9]=k;a[11]=w;a[12]=d;a[13]=m;a[14]=t;return this};a.kmMat4Multiply=function(a,b,c){var d=a.mat,k=b.mat,m=c.mat;c=k[0];b=k[1];var p=k[2],r=k[3],t=k[4],u=k[5],x=k[6],w=k[7],E=k[8],n=k[9],M=k[10],B=k[11],D=k[12],N=k[13],y=k[14],k=k[15],C=m[0],z=m[1],I=m[2],G=m[3],H=m[4],X=m[5],Y=m[6],R=m[7],J=m[8],Z=m[9],T=m[10],da=m[11],V=m[12],ba=m[13],ha=m[14],m=m[15];d[0]=C*c+z*t+I*E+G*D;d[1]=C*b+z*u+I*n+G*N;d[2]=C*p+z*x+I*M+G*y;d[3]=C*r+z*w+I*B+G*k;d[4]=H*c+X*t+Y*E+R*
D;d[5]=H*b+X*u+Y*n+R*N;d[6]=H*p+X*x+Y*M+R*y;d[7]=H*r+X*w+Y*B+R*k;d[8]=J*c+Z*t+T*E+da*D;d[9]=J*b+Z*u+T*n+da*N;d[10]=J*p+Z*x+T*M+da*y;d[11]=J*r+Z*w+T*B+da*k;d[12]=V*c+ba*t+ha*E+m*D;d[13]=V*b+ba*u+ha*n+m*N;d[14]=V*p+ba*x+ha*M+m*y;d[15]=V*r+ba*w+ha*B+m*k;return a};b.multiply=function(a){var b=this.mat,c=a.mat;a=b[0];var d=b[1],k=b[2],m=b[3],p=b[4],r=b[5],t=b[6],u=b[7],x=b[8],w=b[9],E=b[10],n=b[11],M=b[12],B=b[13],D=b[14],N=b[15],y=c[0],C=c[1],z=c[2],I=c[3],G=c[4],H=c[5],X=c[6],Y=c[7],R=c[8],J=c[9],Z=
c[10],T=c[11],da=c[12],V=c[13],ba=c[14],c=c[15];b[0]=y*a+C*p+z*x+I*M;b[1]=y*d+C*r+z*w+I*B;b[2]=y*k+C*t+z*E+I*D;b[3]=y*m+C*u+z*n+I*N;b[4]=G*a+H*p+X*x+Y*M;b[5]=G*d+H*r+X*w+Y*B;b[6]=G*k+H*t+X*E+Y*D;b[7]=G*m+H*u+X*n+Y*N;b[8]=R*a+J*p+Z*x+T*M;b[9]=R*d+J*r+Z*w+T*B;b[10]=R*k+J*t+Z*E+T*D;b[11]=R*m+J*u+Z*n+T*N;b[12]=da*a+V*p+ba*x+c*M;b[13]=da*d+V*r+ba*w+c*B;b[14]=da*k+V*t+ba*E+c*D;b[15]=da*m+V*u+ba*n+c*N;return this};a.getMat4MultiplyValue=function(a,b){var c=a.mat,d=b.mat,k=new Float32Array(16);k[0]=c[0]*
d[0]+c[4]*d[1]+c[8]*d[2]+c[12]*d[3];k[1]=c[1]*d[0]+c[5]*d[1]+c[9]*d[2]+c[13]*d[3];k[2]=c[2]*d[0]+c[6]*d[1]+c[10]*d[2]+c[14]*d[3];k[3]=c[3]*d[0]+c[7]*d[1]+c[11]*d[2]+c[15]*d[3];k[4]=c[0]*d[4]+c[4]*d[5]+c[8]*d[6]+c[12]*d[7];k[5]=c[1]*d[4]+c[5]*d[5]+c[9]*d[6]+c[13]*d[7];k[6]=c[2]*d[4]+c[6]*d[5]+c[10]*d[6]+c[14]*d[7];k[7]=c[3]*d[4]+c[7]*d[5]+c[11]*d[6]+c[15]*d[7];k[8]=c[0]*d[8]+c[4]*d[9]+c[8]*d[10]+c[12]*d[11];k[9]=c[1]*d[8]+c[5]*d[9]+c[9]*d[10]+c[13]*d[11];k[10]=c[2]*d[8]+c[6]*d[9]+c[10]*d[10]+c[14]*
d[11];k[11]=c[3]*d[8]+c[7]*d[9]+c[11]*d[10]+c[15]*d[11];k[12]=c[0]*d[12]+c[4]*d[13]+c[8]*d[14]+c[12]*d[15];k[13]=c[1]*d[12]+c[5]*d[13]+c[9]*d[14]+c[13]*d[15];k[14]=c[2]*d[12]+c[6]*d[13]+c[10]*d[14]+c[14]*d[15];k[15]=c[3]*d[12]+c[7]*d[13]+c[11]*d[14]+c[15]*d[15];return k};a.kmMat4Assign=function(b,c){if(b===c)return a.log("cc.kmMat4Assign(): pOut equals pIn"),b;var d=b.mat,h=c.mat;d[0]=h[0];d[1]=h[1];d[2]=h[2];d[3]=h[3];d[4]=h[4];d[5]=h[5];d[6]=h[6];d[7]=h[7];d[8]=h[8];d[9]=h[9];d[10]=h[10];d[11]=
h[11];d[12]=h[12];d[13]=h[13];d[14]=h[14];d[15]=h[15];return b};b.assignFrom=function(b){if(this===b)return a.log("cc.mat.Matrix4.assignFrom(): mat4 equals current matrix"),this;var c=this.mat;b=b.mat;c[0]=b[0];c[1]=b[1];c[2]=b[2];c[3]=b[3];c[4]=b[4];c[5]=b[5];c[6]=b[6];c[7]=b[7];c[8]=b[8];c[9]=b[9];c[10]=b[10];c[11]=b[11];c[12]=b[12];c[13]=b[13];c[14]=b[14];c[15]=b[15];return this};b.equals=function(b){if(this===b)return a.log("cc.kmMat4AreEqual(): pMat1 and pMat2 are same object."),!0;var c=this.mat;
b=b.mat;for(var d=a.math.EPSILON,h=0;16>h;h++)if(!(c[h]+d>b[h]&&c[h]-d<b[h]))return!1;return!0};a.math.Matrix4.createByRotationX=function(b,c){c=c||new a.math.Matrix4;var d=c.mat;d[0]=1;d[3]=d[2]=d[1]=0;d[4]=0;d[5]=Math.cos(b);d[6]=Math.sin(b);d[7]=0;d[8]=0;d[9]=-Math.sin(b);d[10]=Math.cos(b);d[11]=0;d[14]=d[13]=d[12]=0;d[15]=1;return c};a.math.Matrix4.createByRotationY=function(b,c){c=c||new a.math.Matrix4;var d=c.mat;d[0]=Math.cos(b);d[1]=0;d[2]=-Math.sin(b);d[3]=0;d[7]=d[6]=d[4]=0;d[5]=1;d[8]=
...
var MIN_SPEED=5,MAX_SPEED=20,PokerCardSprite=cc.Sprite.extend({ctor:function(a){this._super();this.model=a.model;this.reverse=a.reverse;this.numberSprite=new cc.Sprite;this.numberSprite.attr({x:25,y:55});this.setName(this.model.cid);this.addChild(this.numberSprite,0);var b=this;this.listener=cc.EventListener.create({event:cc.EventListener.TOUCH_ONE_BY_ONE,swallowTouches:!0,onTouchBegan:function(a,b){var e=b.getCurrentTarget(),f=e.convertToNodeSpace(a.getLocation()),g=e.getContentSize(),g=cc.rect(0,
0,g.width,g.height);return cc.rectContainsPoint(g,f)?(e.stopAllActions(),e.speedX=0,e.speedY=e.y>=cc.winSize.height/2?1:-1,!0):!1},onTouchMoved:function(a,d){var e=d.getCurrentTarget();if(!e.alreadyTaken){var f=a.getDelta();b.x+=f.x;b.y+=f.y;b.speedX=f.x;b.speedY=f.y;f=cc.winSize.height/2;0===b.rotation&&b.y>=f?e.runAction(new cc.rotateTo(times.rotateDirection,180)):180===b.rotation&&b.y<f&&e.runAction(new cc.rotateTo(times.rotateDirection,0))}},onTouchEnded:function(a,d){var e=d.getCurrentTarget();
if(!e.alreadyTaken)if(e.y<dimens.player1Y)e.playerTakeCard.call(e,gameModel.player1);else if(e.y>dimens.player2Y)e.playerTakeCard.call(e,gameModel.player2);else{var f=cc.winSize.height/2,g=dimens.player1Y,h=dimens.player2Y,k=Math.max(0.1,Math.sqrt(b.speedX*b.speedX+b.speedY*b.speedY)),m=30*Math.min(MAX_SPEED,Math.max(MIN_SPEED,k)),k=[];if(0<b.speedY){if(e.y<f){var p=b.getCrossPointX.call(b,f),r=Math.sqrt((e.x-p)*(e.x-p)+(e.y-f)*(e.y-f))/m;k.push(new cc.moveTo(r,p,f));var t=b.getCrossPointX.call(b,
h),f=Math.sqrt((t-p)*(t-p)+(h-f)*(h-f))/m;k.push(new cc.spawn(new cc.rotateTo(times.rotateDirection,180),new cc.moveTo(f,t,h)))}else t=b.getCrossPointX.call(b,h),r=Math.sqrt((e.x-t)*(e.x-t)+(e.y-h)*(e.y-h))/m,k.push(new cc.moveTo(r,t,h));t>-dimens.card_size.width/2&&t<cc.winSize.width+dimens.card_size.width/2?k.push(new cc.callFunc(function(){e.playerTakeCard.call(e,gameModel.player2)},b)):k.push(new cc.callFunc(function(){gameModel.destroyCard(e.model)},b))}else 0>b.speedY?(e.y>f?(p=b.getCrossPointX.call(b,
f),r=Math.sqrt((e.x-p)*(e.x-p)+(e.y-f)*(e.y-f))/m,k.push(new cc.moveTo(r,p,f)),t=b.getCrossPointX.call(b,g),f=Math.sqrt((t-p)*(t-p)+(g-f)*(g-f))/m,k.push(new cc.spawn(new cc.rotateTo(times.rotateDirection,0),new cc.moveTo(f,t,g)))):(t=b.getCrossPointX.call(b,g),r=Math.sqrt((e.x-t)*(e.x-t)+(e.y-g)*(e.y-g))/m,k.push(new cc.moveTo(r,t,g))),t>-dimens.card_size.width/2&&t<cc.winSize.width+dimens.card_size.width/2?k.push(new cc.callFunc(function(){e.playerTakeCard.call(e,gameModel.player1)},b)):k.push(new cc.callFunc(function(){gameModel.destroyCard(e.model)},
b))):0==b.speedY&&(0>b.speedX?(r=(e.x- -dimens.card_size.width/2)/m,k.push(new cc.moveTo(r,-dimens.card_size.width/2,e.y)),k.push(new cc.callFunc(function(){gameModel.destroyCard(e.model)},b))):0<b.speedX&&(r=(cc.winSize.width+dimens.card_size.width/2-e.x)/m,k.push(new cc.moveTo(r,cc.winSize.width+dimens.card_size.width/2,e.y)),k.push(new cc.callFunc(function(){gameModel.destroyCard(e.model)},b))));k.length&&e.runAction(new cc.sequence(k))}}})},playerTakeCard:function(a){a.canTakeCard()?(this.isNewHand=
!0,this.setTag(0),a.addHand(this.model),this.alreadyTaken=!0,cc.eventManager.removeListener(this.listener)):(this.stopAllActions(),gameModel.destroyCard(this.model))},getCrossPointX:function(a){return(a-this.y)*this.speedX/this.speedY+this.x},render:function(){this.setSpriteFrame(cc.spriteFrameCache.getSpriteFrame("card-"+SUIT_ARRAY[this.model.get("suit")]+".png"));this.numberSprite.setSpriteFrame(cc.spriteFrameCache.getSpriteFrame("number-"+this.model.get("number")+".png"))},onEnter:function(){this._super();
this.initEvent();this.render()},onExit:function(){this.closeEvent();this._super()},initEvent:function(){cc.eventManager.addListener(this.listener,this);this.model.on("destroy",this.onDestroy,this)},closeEvent:function(){cc.eventManager.removeListener(this.listener);this.model.off("destroy",this.onDestroy)},onDestroy:function(){this.removeFromParent(!0)},getFlipToFrontSequence:function(){return new cc.sequence(new cc.scaleTo(times.flip/2,0,1),new cc.callFunc(function(){this.setSpriteFrame(cc.spriteFrameCache.getSpriteFrame("card-"+
SUIT_ARRAY[this.model.get("suit")]+".png"));this.numberSprite.setVisible(!0)},this),new cc.scaleTo(times.flip/2,1,1))},getFlipToBackSequence:function(){return new cc.sequence(new cc.scaleTo(times.flip/2,0,1),new cc.callFunc(function(){this.setSpriteFrame(cc.spriteFrameCache.getSpriteFrame("card-back.png"));this.numberSprite.setVisible(!1)},this),new cc.scaleTo(times.flip/2,1,1))}});var STRAIGHT_MAP={"14-3-2":!0,"4-3-2":!0,"5-4-3":!0,"6-5-4":!0,"7-6-5":!0,"8-7-6":!0,"9-8-7":!0,"10-9-8":!0,"11-10-9":!0,"12-11-10":!0,"13-12-11":!0,"14-13-12":!0,"14-4-3-2":!0,"5-4-3-2":!0,"6-5-4-3":!0,"7-6-5-4":!0,"8-7-6-5":!0,"9-8-7-6":!0,"10-9-8-7":!0,"11-10-9-8":!0,"12-11-10-9":!0,"13-12-11-10":!0,"14-13-12-11":!0,"14-5-4-3-2":!0,"6-5-4-3-2":!0,"7-6-5-4-3":!0,"8-7-6-5-4":!0,"9-8-7-6-5":!0,"10-9-8-7-6":!0,"11-10-9-8-7":!0,"12-11-10-9-8":!0,"13-12-11-10-9":!0,"14-13-12-11-10":!0},PLAYER_POSITION_DOWN=
...
var c,k,m;m=(d-cc.winSize.width/2)/400;c=400*Math.sin(m)+cc.winSize.width/2;k=400*Math.cos(m)+b-400+20;m*=50;this.model.get("position")==PLAYER_POSITION_UP&&(k=cc.winSize.height-k,c=cc.winSize.width-c,m+=180);!a||a.x==d&&a.y==b||(a.movingAction&&a.stopAction(a.movingAction),a.movingAction=new cc.spawn(new cc.moveTo(times.card_sort,c,k),new cc.rotateTo(times.card_sort,m)),a.isNewHand&&(this.showHand||a.runAction(a.getFlipToBackSequence()),a.isNewHand=!1),a.runAction(a.movingAction));a.zIndex=f;f++;
d+=e},this);a.length==MAX_HAND&&gameModel.trigger("start-countdown",gameModel)}},toggleHand:function(){if("compare"!==gameModel.get("status")){this.showHand=!this.showHand;var a=this.model.get("hands");_.each(a,function(a){a=this.getParent().getChildByName(a.cid);this.showHand?a.runAction(a.getFlipToFrontSequence()):a.runAction(a.getFlipToBackSequence())},this)}},forceShowHand:function(){var a=this.model.get("hands");_.each(a,function(a){a=this.getParent().getChildByName(a.cid);a.numberSprite.isVisible()||
...
function(){},this);a.attr({x:0,y:cc.winSize.height/2,anchorX:0,anchorY:0.5});a=new cc.Menu([a]);a.x=0;a.y=0;this.addChild(a);this.betRateLabel1=new cc.LabelTTF("","Arial",30);this.betRateLabel1.attr({color:colors.tableLabel,x:cc.winSize.width,y:cc.winSize.height/2-15,anchorX:1});this.addChild(this.betRateLabel1,0);this.betRateLabel2=new cc.LabelTTF("","Arial",30);this.betRateLabel2.attr({color:colors.tableLabel,x:cc.winSize.width,y:cc.winSize.height/2+15,rotation:180,anchorX:0});this.addChild(this.betRateLabel2,
...
_.sample([0,1]),c=_.sample([0,1]),a=a.get("list");_.each(a,function(a){var e=this.model.drawCard();if(null!=e){var f=new PokerCardSprite({model:e});f.attr({x:b?cc.winSize.width-a.start.x:a.start.x,y:a.start.y});f.y>=cc.winSize.height/2&&(f.rotation=180);this.addChild(f);var g=a.moveTime;14===e.get("number")&&(g/=2);f.runAction(new cc.sequence(new cc.delayTime(a.time),new cc.moveTo(g,b?cc.winSize.width-a.end.x:a.end.x,a.end.y),new cc.callFunc(function(){gameModel.destroyCard(e)},this)));var h=this.model.drawCard();
if(null!=h){var f=new PokerCardSprite({model:h}),k,m;c?(f.attr({x:a.start.x,y:cc.winSize.height-a.start.y}),k=a.end.x):(f.attr({x:cc.winSize.width-a.start.x,y:cc.winSize.height-a.start.y}),k=cc.winSize.width-a.end.x);m=cc.winSize.height-a.end.y;f.y>=cc.winSize.height/2&&(f.rotation=180);this.addChild(f);g=a.moveTime;14===h.get("number")&&(g/=2);f.runAction(new cc.sequence(new cc.delayTime(a.time),new cc.moveTo(g,k,m),new cc.callFunc(function(){gameModel.destroyCard(h)},this)))}}},this)},initAudio:function(){var a=
cc.sys.localStorage.getItem("sound");this.sound=null!=a?a:0.5;cc.audioEngine.setEffectsVolume(this.sound)}}),GameModel=Backbone.Model.extend({initialize:function(){this.countDown=this.maxCountDown=60;this.generateCardCountDown=0;this.set("betRate",1);this.set("status","ready");this.totalTime=0;this.cidToModel={};this.player1=new PlayerModel({position:PLAYER_POSITION_DOWN,playerType:"player"});this.player2=new PlayerModel({position:PLAYER_POSITION_UP,playerType:"player"});this.patternPool=[new PatternModel,
new Pattern2Model,new Pattern3Model,new Pattern4Model,new Pattern5Model,new Pattern6Model]},newDeck:function(){this.deck=newDeck();this.discardDeck=[]},drawCard:function(){var a=this.deck.pop();a||(this.deck=_.shuffle(this.discardDeck),this.discardDeck=[],a=this.deck.pop());a&&(this.cidToModel[a.cid]=a,a._owned=!1);return a},clearNotOwnedCards:function(){var a=[];_.each(this.cidToModel,function(b){null==b||b._owned||a.push(b)},this);_.each(a,function(a){this.destroyCard(a)},this)},clearCards:function(){var a=
[];_.each(this.cidToModel,function(b){null!=b&&a.push(b)},this);_.each(a,function(a){this.destroyCard(a)},this);this.cidToModel={}},destroyCard:function(a){this.discardDeck.push(new PokerCardModel({number:a.get("number"),suit:a.get("suit")}));a.destroy();delete this.cidToModel[a.cid]},getPattern:function(){return _.sample(this.patternPool)}}),MainScene=cc.Scene.extend({onEnter:function(){this._super();if(!window.gameModel){window.gameModel=new GameModel;var a=new MainLayer({model:gameModel,need_read_fight:!0});
this.addChild(a)}}});cc.game.onStart=function(){!cc.sys.isNative&&document.getElementById("cocosLoading")&&document.body.removeChild(document.getElementById("cocosLoading"));cc.view.enableRetina(!1);cc.view.adjustViewPort(!0);cc.view.setDesignResolutionSize(450,800,cc.ResolutionPolicy.SHOW_ALL);cc.view.resizeWithBrowserSize(!0);cc.LoaderScene.preload(g_resources,function(){cc.spriteFrameCache.addSpriteFrames(res.game_plist);cc.director.runScene(new MainScene)},this)};cc.game.run();